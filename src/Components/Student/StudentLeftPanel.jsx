...
import {StudentContext} from "../Student/StudentDashboard.jsx";



const StudentLeftPanel = ({name, id}) => {
    const navigate = useNavigate();
    const {selectedItem, setSelectedItem } = useContext(StudentContext)
    return (
        <div style={bgcolor2} className="leftSidePanel border-2  border-red-300 rounded-lg flex flex-col p-3 w-2/12 h-full items-center">
            ...
            <div className="w-7/12 ">
                <img className="admin-profile" alt="Student profile" src={require('../../img/student.png')} />
            </div>
            <h2 className="font-bold">{name}</h2>
            <h2 className="font-bold">Student Id : {id}</h2>
